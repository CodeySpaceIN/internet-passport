...
import { getAdminOrganizationsData, getTrustSummaryForTarget } from "@/lib/data/admin-console";
...
export default async function AdminOrganizationsPage({ searchParams }: PageProps) {
  ...
  const selectedId = readParam(params, "organizationId");
  const organizations = await getAdminOrganizationsData(tenantId, q || undefined);
  const selected = organizations.find((item) => item.id === selectedId) ?? organizations[0] ?? null;
  const trust = selected ? await getTrustSummaryForTarget(tenantId, { organizationId: selected.id }) : null;

  return (
    <div className="grid gap-4 xl:grid-cols-[1.2fr_1fr]">
        <AdminTableCard title="Organization search">
          <form method="GET" className="mb-3 flex gap-2">
            <input name="q" defaultValue={q} placeholder="Search org name or slug" className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm" />
            ...
            <tbody>
              {organizations.map((org) => (
                <tr key={org.id} className="border-b border-slate-100">
                  ...
                  <td className="px-2 py-2 text-slate-700">{org.trustScores[0]?.score ?? "-"}</td>
                  <td className="px-2 py-2">
                    <a href={`/admin/orgs?organizationId=${org.id}${q ? `&q=${encodeURIComponent(q)}` : ""}`} className="text-xs text-slate-700 underline">
                      Open
                    </a>
                  </td>
                </tr>
        ...
        <AdminTableCard title="Organization inspection">
          {!selected ? (
            <p className="text-sm text-slate-600">No organization found.</p>
          ) : (
            <div className="space-y-3 text-sm">
              ...
              <div>
                <p className="mb-1 text-xs uppercase tracking-[0.12em] text-slate-500">Domain history</p>
                <ul className="space-y-1 text-slate-700">
              ...
              <form action={suspendEntityAction} className="space-y-2">
                <input type="hidden" name="entityType" value="org" />
